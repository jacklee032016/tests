...
//	#define	DEBUG(fmt, args...)	{printf("[%s-%u] DEBUG: " fmt EXT_NEW_LINE, __FILE__, __LINE__, ## args);}
	#define	DEBUGF(debug, format, message...)		do { \
                               if ( \
                                   ((debug) & EXT_DBG_ON) && \
	...
	#define	INFOF(format, message...)			{SYS_PRINT(ANSI_COLOR_CYAN "%s [INFO,%s]: [%s-%u.%s()]:" format ANSI_COLOR_RESET EXT_NEW_LINE, sysTimestamp(), sysTaskName(), __FILE__, __LINE__, __FUNCTION__, ##message);}
	
	#define	ERRORF(format, message...)			{SYS_PRINT(ERROR_TEXT_BEGIN "%s [ERR, %s]: [%s-%u.%s()]:" format ERROR_TEXT_END  EXT_NEW_LINE, sysTimestamp(), sysTaskName(), __FILE__, __LINE__, __FUNCTION__, ##message);}


//	#define	ASSERT(x)				{printf("Assertion \"%s\" failed at line %d in %s\n", x, __LINE__, __FILE__); while(1);}
	#define	ASSERT(x, format, msg...)			{if((x)==0) {SYS_PRINT(ERROR_TEXT_BEGIN"%s %s: ASSERT: [%s-%u]:" format ERROR_TEXT_END EXT_NEW_LINE, sysTimestamp(),  sysTaskName(), __FILE__, __LINE__ , ##msg);while(0){};}}
	#define	ABORT(fmt, args... )					SYS_PRINT("%s %s: ABORT in [" __FILE__ "-%u]:" fmt EXT_NEW_LINE, sysTimestamp(), sysTaskName(), __LINE__, ##args );while(1){}

	#define TRACE() 								_TRACE_OUT(EXT_NEW_LINE )
#else

	#define	DEBUGF(debug, format, message...)		{}

	#define	INFOF(format, message...)				{SYS_PRINT(ANSI_COLOR_CYAN "%s [INFO, %s] "format ANSI_COLOR_RESET EXT_NEW_LINE, sysTimestamp(), sysTaskName(), ##message );}

	#define	ERRORF(format, message...)				{SYS_PRINT(ERROR_TEXT_BEGIN "%s [ERR, %s] "format ERROR_TEXT_END EXT_NEW_LINE, sysTimestamp(), sysTaskName(), ##message);}
	
//	#define	ASSERT(x)				{while (1);}
	#define	ASSERT(x, format, msg...)				{}
